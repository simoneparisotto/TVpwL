function [x,res_final,iter,res,bound] = primal_dual(x, K, KS, ProxFS, ProxG, options)
...
for iter = 1:niter
    ...
    % primal residual
    p_res = (x_old-x)/tau - (xstar_old-xstar) ; %- norms((y_old-y)/sigma,2,3).*options.gamma;
    p = sum(abs(p_res));
    % dual residual
    ...
    bound(iter) = norm(x(:)-x0(:),2)-options.bound;
    if true
